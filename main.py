...
def addOffer():
    ...
    if result00:
        ...
        if result0:
            ...
            values=(data["offerNo"],data["date"],data["validityDate"],alternate,qty,result0[1],data["unitPrice"],totalPrice,discount,data["offerDiscount"],offerFreightCharges,data["offerFreightCharges"],finalPrice,offerFrieghtChargersWithUnitPrice,str(data["deliveryDate"]),"Not Recieved",data["rfqNo"],data["partNo"],code)
            cursor.execute(query,values)
            db.commit()
...
def reviseOffer():
    ...
    if result0:
        ...
        ##print(query)
        values=(data["date"],data["validityDate"],alternate,qty,result0[1],data["unitPrice"],totalPrice,discount,data["offerDiscount"],offerFreightCharges,data["offerFreightCharges"],finalPrice,offerFrieghtChargersWithUnitPrice,str(data["deliveryDate"]),"Not Recieved",data["rfqNo"],data["partNo"],code,data["offerNo"])
        cursor.execute(query,values)
        db.commit()
        query3="update itemStatus set status=%s where partNo=%s and rfqNo=%s"
        cursor.execute(query3,("Recieved",result0[0],data["rfqNo"]))
